  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 bg-white dark:bg-[#181818] rounded-[28px] border border-gray-200/80 dark:border-neutral-800 shadow-xs flex items-center gap-4">
          <div className="p-3 bg-blue-50 dark:bg-[#242424] text-[#2563EB] dark:text-blue-400 rounded-2xl shrink-0">
            <Mail className="w-6 h-6" />
          </div>
          <div>
            <h4 className="text-sm font-extrabold text-[#111827] dark:text-white">Email Support</h4>
            <p className="text-xs text-gray-500 dark:text-neutral-400">support@googlemerch.com</p>
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 mt-1 block">Avg. response 2 hrs</span>
          </div>
        </div>

        <div className="p-6 bg-white dark:bg-[#181818] rounded-[28px] border border-gray-200/80 dark:border-neutral-800 shadow-xs flex items-center gap-4">
          <div className="p-3 bg-emerald-50 dark:bg-[#242424] text-emerald-600 dark:text-emerald-400 rounded-2xl shrink-0">
            <MessageSquare className="w-6 h-6" />
          </div>
          <div>
            <h4 className="text-sm font-extrabold text-[#111827] dark:text-white">Live Chat</h4>
            <p className="text-xs text-gray-500 dark:text-neutral-400">Available 24/7 in app</p>
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 mt-1 block">Instant Assistance</span>
          </div>
        </div>

        <div className="p-6 bg-white dark:bg-[#181818] rounded-[28px] border border-gray-200/80 dark:border-neutral-800 shadow-xs flex items-center gap-4">
          <div className="p-3 bg-indigo-50 dark:bg-[#242424] text-indigo-600 dark:text-indigo-400 rounded-2xl shrink-0">
            <Phone className="w-6 h-6" />
          </div>
          <div>
            <h4 className="text-sm font-extrabold text-[#111827] dark:text-white">Toll-Free Phone</h4>
            <p className="text-xs text-gray-500 dark:text-neutral-400">1-800-555-MERCH</p>
            <span className="text-[10px] font-bold text-gray-500 dark:text-neutral-400 mt-1 block">Mon-Fri 8am-8pm EST</span>
          </div>
        </div>
      </div>
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                  selectedCat === cat.id
                    ...
                    : 'bg-white dark:bg-[#222222] border border-gray-200 dark:border-neutral-700 text-gray-500 dark:text-neutral-300 hover:text-gray-900 dark:hover:text-white'
                }`}
              >
                {cat.label}
              </button>
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-white dark:bg-[#181818] rounded-2xl border border-gray-200/80 dark:border-neutral-800 overflow-hidden transition-all shadow-2xs"
              >
                <button
                  onClick={() => setOpenIdx(isOpen ? null : idx)}
                  className="w-full p-5 text-left font-extrabold text-xs sm:text-sm text-[#111827] dark:text-white flex items-center justify-between gap-4 cursor-pointer hover:bg-gray-50/50 dark:hover:bg-[#202020]"
                >
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      ...
                      className="px-5 pb-5 text-xs text-gray-500 dark:text-neutral-400 leading-relaxed border-t border-gray-100 dark:border-neutral-800 pt-3"
                    >
                      {faq.answer}
                    </motion.div>
                  )}
      ...
      <section className="bg-white dark:bg-[#181818] rounded-[32px] border border-gray-200/80 dark:border-neutral-800 p-8 sm:p-12 shadow-xs grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="pt-4 space-y-3 text-xs text-[#111827] dark:text-neutral-200">
        ...
        <div className="lg:col-span-7">
          {submitted ? (
            <div className="p-8 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 rounded-3xl text-center space-y-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-[#111827] dark:text-neutral-200">Your Name</label>
                  <input
                    type="text"
                    placeholder="Alex Mercer"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-[#222222] border border-gray-200 dark:border-neutral-700 text-[#111827] dark:text-white placeholder:text-gray-400 rounded-2xl text-xs focus:outline-none focus:border-[#2563EB]"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-[#111827] dark:text-neutral-200">Email Address *</label>
                  <input
                    type="email"
                    required
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-[#111827] dark:text-neutral-200">Subject</label>
                <select
                  value={formData.subject}
                  onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-[#222222] border border-gray-200 dark:border-neutral-700 text-[#111827] dark:text-white rounded-2xl text-xs focus:outline-none focus:border-[#2563EB]"
                >
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-[#111827] dark:text-neutral-200">Message *</label>
                <textarea
                  required
                  rows={4}
